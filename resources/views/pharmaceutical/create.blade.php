    ...
    <section class="container py-5">
        <div class="container my-5 " dir="rtl">
            <div class="card shadow p-4 mx-auto position-relative" style="max-width: 600px;">
                <a href="{{route('medicines.index')}}" class="btn-close position-absolute top-0 end-0 m-3" aria-label="إغلاق"></a>
                <h2 class="text-center mb-4 fw-bold">إضافة دواء</h2>
                <form action="{{ route('medicines.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select input-form" id="status" name="status">
                            ...
                            <option value="low" {{ old('status') == 'low' ? 'selected' : '' }}>قليل</option>
                            <option value="unavailable" {{ old('status') == 'unavailable' ? 'selected' : '' }}>غير متوفر
                            </option>
                        </select>
                        @error('status')
                            <div class="alert alert-danger mt-2">{{ $message }}</div>
